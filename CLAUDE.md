...
- 커밋: 한국어 conventional commit (`feat`/`fix`/`refactor`/`docs`/`chore`). **커밋 메시지에 Claude 서명(Co-Authored-By 등) 넣지 말 것.** → **`/commit` 스킬 사용.** **Claude 는 사용자가 명시적으로 요청할 때만 커밋한다** (임의 커밋 금지 — `.claude/settings.json` 의 ask 규칙으로도 강제됨).
