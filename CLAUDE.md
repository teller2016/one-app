...
│       └── notify/              #  데스크톱 알림 (공통 인프라)
│           └── notify.ts        #    Notification 래퍼 + 클릭 시 창 포커스·섹션 이동
...
- **알림** (`main/features/notify`): 데스크톱 알림 공통 인프라. `notify({title, body, section})` 호출 시 Electron `Notification` 표시, 클릭하면 앱 창을 앞으로 + `app:navigate` IPC 로 해당 섹션 이동(App.tsx `onNavigate` 구독). 창 참조는 `main.ts`에서 `setNotifyWindow()` 로 등록. 사용처: ①배포 완료/실패 알림(환경설정 `settings.notifyDeploy` on/off) ②출퇴근 리마인더. 환경설정에 **테스트 알림 버튼**(`notify:test` IPC → `window.oneApp.testNotification()`)이 있어 모양·권한 확인 가능. 새 알림이 필요하면 이 모듈을 재사용.
- **출퇴근 리마인더** (`main/features/attendance/scheduler.ts` + `reminders.ts`): 환경설정에서 **요일별(월~금)로 출근·퇴근 알림 시각**을 각각 지정(체크박스+시각). 메인 스케줄러가 매 30초 현재 시각을 확인해, 설정 시각(±2분, 슬립 대비)이 되면 근태 상태를 조회(`runAttendance('status')`)하고 **이미 찍었으면 건너뛰고 안 찍었을 때만** 알림(스마트 스킵). 상태 확인 실패(계정 없음·VPN 등)면 놓치지 않도록 알림을 띄운다. 평일만, 하루 한 번(중복 방지). 설정은 `userData/reminders.json`(평문). 스케줄러는 저장값을 매 tick 읽으므로 저장 후 재시작 불필요.
