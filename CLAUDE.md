...
- **폰 미러링** (`renderer/features/mirror` + `main/features/mirror`): 사이드바 하단 위젯(VPN 아래). Homebrew `scrcpy`(선택 의존성)를 `-d --turn-screen-off` 로 spawn — 바탕화면 'Mirror USB.app' 이식. `adb devices -l` 로 USB 기기 모델명을 표시하고 기기 없으면 시작 버튼 비활성. 미러 창을 닫으면 exit 이벤트로 위젯 상태 자동 갱신(`mirror:changed`), 비정상 종료는 stderr 마지막 줄을 에러로 표시. 앱 종료 시 scrcpy 도 함께 종료됨(VPN 과 달리 독립 유지 안 함). 설정·저장 없음.
