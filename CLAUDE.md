...
- **⚠️ 그룹웨어 로그인은 반드시 `main/lib/groupware.ts` 의 `withGroupwareLogin()` 으로 감쌀 것** — 같은 계정으로 **거의 동시에 로그인하면 서버가 뒤쪽을 거부**해 로그인 페이지로 되돌려보낸다(2026-07-30 실측: 앱 시작 시 메일 위젯+근태 위젯이 각자 headless 브라우저로 동시 로그인 → 메일이 4/4 실패, "계정 정보를 확인하세요" 라는 오해성 메시지로 표시됨. 중복 로그인 확인창은 뜨지 않아 다이얼로그 처리로는 해결 불가). 적용 대상은 로그인하는 5개 기능 — 메일(`session.ts`)·근태(`attend.ts`)·주간보고(`collect.ts`)·야근결재(`submit.ts`)·일정 등록(`runMacro.ts`). **로그인 구간만** 감싸고 이후 조회·스크래핑은 감싸지 않아 병렬로 돈다(직렬화 후 3/3 정상). 단, `standalone/` 은 별도 프로세스라 이 큐가 적용되지 않는다.
  - 📌 **개선 여지(미적용, 2026-07-30 실측 완료)**: 그룹웨어 세션은 **쿠키 주입으로 재사용 가능**하다 — 로그인한 브라우저에서 쿠키를 뽑아 다른 브라우저에 `Network.setCookies` 로 넣고 `userMain.do` 로 직행하면 로그인 화면으로 튕기지 않고 근태 위젯까지 정상 판독(2회 모두 성공). 소요시간 **로그인 경로 4.8/4.5초 → 주입 경로 1.3초**(약 3.5배 빠름). 쿠키는 `JSESSIONID` **2개**(`gw.forbiz.co.kr` 경로 `/gw` + `.forbiz.co.kr` 경로 `/`)이므로, 공용 세션으로 승격할 땐 메일의 `"name=value; …"` 합친 문자열이 아니라 **도메인·경로가 붙은 쿠키 객체 그대로 보관**해야 한다.
