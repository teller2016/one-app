...
- **알림** (`main/features/notify`): 데스크톱 알림 공통 인프라. `notify({title, body, section})` 호출 시 Electron `Notification` 표시, 클릭하면 앱 창을 앞으로 + `app:navigate` IPC 로 해당 섹션 이동(App.tsx `onNavigate` 구독). 창 참조는 `main.ts`에서 `setNotifyWindow()` 로 등록. 사용처: ①배포 완료/실패 알림(환경설정 `settings.notifyDeploy` on/off) ②출퇴근 리마인더. 새 알림이 필요하면 이 모듈을 재사용.
