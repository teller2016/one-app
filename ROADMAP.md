...
| 프로젝트 | 섹션 | 배포·PR·터미널이 참조하는 프로젝트 중앙 레지스트리(경로·저장소·Jira 키) |
| 딥링크 | 섹션 | applink.kr 생성 |
| 결재 | 섹션+모달 | 야근·휴가·지출결의서 — 그룹웨어 결재 자동 작성(숨은 BrowserWindow). 야근은 출퇴근 위젯에서도 진입 |
| 일정 등록 | 섹션 | 그룹웨어 하루 일정 자동 입력 + 노션 투입시간 기록 |
...
| 터미널 | 섹션 | tmux 백엔드 + xterm(WebGL), 워크스페이스·워크트리별 세션·분할·프리셋, **MO(모바일) 브라우저와 같은 세션 공유** |
| 변경사항 | 드로어+MO 탭 | 선택 워크트리의 git 상태·diff·커밋·푸시 (터미널 옆 드로어 ⌘B, 전체 화면 사이드-바이-사이드 diff) |
| MO 앱 | 모바일 셸 | 폰 브라우저용 셸 — 터미널 외 근태·배포·Jira·PR·메일·변경 탭 (렌더러 재사용, RPC shim) |
| 알림 | 인프라 | 알럿 알림 공통 모듈 (메뉴바 트레이는 2026-09-16 제거 — 안 쓴다) |
...
- [x] **MO 확장** — MO 앱 셸(`src/mobile-app`)에 근태·배포·Jira·PR·메일·변경 탭으로 완료(2026-08)
- [ ] **스크립트·액션 런처** — 자주 쓰는 `.command`/스크립트/API 호출 버튼 모음
- [ ] **웹 대시보드 임베드** — 젠킨스·NAS 등 webview 탭
- [ ] **휴가·연차 조회** — 휴가 **결재**는 결재 섹션으로 완료, 잔여 연차 **조회**는 미구현(그룹웨어 자동화 인프라 재사용)
