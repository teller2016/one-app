...
async function patchPopupOpener(page: Page) {
  await evalInPage(
    ...
    ['/exp/ex/expend/code/UserCmmCodePop.do'],
  );
}
...
export async function runExpendDraft(
  ...
  try {
    step('그룹웨어 로그인 중…');
    await login(shell, account);

    step('결재양식 목록 여는 중…');
    await goto(shell, EXPEND_CONFIG.formListUrl);
    if (isLoginPage(shell)) {
      throw new Error('그룹웨어 로그인 실패 — 설정의 사번(ID)·비밀번호를 확인하세요.');
    }
    await waitInPage(
      shell,
      (want: string) => {
    ...
    if (page) {
      await releasePage(page).catch(() => undefined);
      await releasePage(shell).catch(() => undefined);
