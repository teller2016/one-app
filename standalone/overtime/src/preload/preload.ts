...
contextBridge.exposeInMainWorld('overtimeApp', {
  ...
  /** 지출결의서(개인) 항목 작성 — 상신하지 않고 화면을 남긴다 */
  runExpend: (input: ExpendInput): Promise<ExpendResult> =>
    ipcRenderer.invoke('expend:run', input),
  /** 남겨둔 자동화 창 닫기 */
  closeKeptWindow: (): Promise<{ closed: boolean }> =>
    ipcRenderer.invoke('window:close-kept'),
  ...
  onProgress: (cb: (progress: OvertimeProgress) => void) => {
    ...
    ipcRenderer.on('draft:progress', listener);
    return () => ipcRenderer.removeListener('draft:progress', listener);
  },
  /** 기본 브라우저로 링크 열기 */
