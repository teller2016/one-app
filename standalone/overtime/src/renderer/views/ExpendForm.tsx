 ...
 */
export function ExpendForm({ configured }: { configured: boolean }) {
  ...
  useEffect(() => {
    void window.overtimeApp.getExpendDefaults().then((d) => {
  ...
  const valid = configured && anySection && parkingValid && dinnerValid;

  const run = async () => {
    setBusy(true);
    setStep('실행 준비 중…');
    setError('');
    const res = await window.overtimeApp.runExpend({
      month,
    ...
    <div className="expend-form">
      ...
      {busy && (
        <p className="progress-line">
          ...
          {step || '진행 중…'}
          <span className="progress-line__note">
            항목마다 찾기 창이 열리고 닫힙니다 — 건드리지 말고 기다려 주세요
          </span>
        </p>
      )}
