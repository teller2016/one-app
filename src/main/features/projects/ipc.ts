...
export function registerProjectsIpc() {
  // 조회·저장은 MO 공유 (PR 의 '빠른 PR' 이 프로젝트 목록을 참조한다)
  handleShared('projects:get', () => listProjects());
  handleShared('projects:save', (input: SaveProjectInput) => saveProject(input));
  handleShared('projects:delete', (id: string) => deleteProject(id));
