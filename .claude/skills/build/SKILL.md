...
ℹ️ **단독 배포판(One App Lite)은 이 스킬의 대상이 아니다** → **`/release`** 를 쓴다. 별도 프로젝트라
자기 폴더(`standalone/lite`)에서 빌드하고 `/Applications` 에 설치하지 않는다 — 팀원에게 건네는 zip 을
만들어 GitHub Releases 에 올린다. 절차·함정은 `.claude/rules/standalone-lite.md` 와 그 폴더 README.
