...
3. **타입 검사(권장)**: `npx tsc --noEmit` 실행. 오류가 있으면 사용자에게 보고하고 커밋 여부를 확인한다.
   아래 '실리는 경로'를 건드렸으면 **`cd standalone/lite && npm run typecheck`** 도 돌린다(루트 tsc 는 단독판을 보지 않는다).
4. **One App Lite 변경 이력**: 단독 배포판(`standalone/lite`)에 **실리는 코드**를 건드린 커밋이면
   ...
   - **실리는 경로**: `standalone/lite/src/**` · `src/main/features/{approval,jira,settings,groupware}/**` · `src/main/lib/**` ·
     `src/shared/**` · `src/renderer/features/approval/**` · `src/renderer/features/jira/components/JiraReportPanel*` ·
     `src/renderer/components/**` · `src/renderer/lib/**` · `src/renderer/styles/**`
