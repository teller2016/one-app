...
- **탭 = 칩, 그룹 = 칩들을 감싼 박스(`tab-pack`)** — `tabView`(TerminalSection)가 `TabItem[]`(single | group)을 만들고, 그룹 멤버 칩들은 인셋 테두리 박스 하나에 담긴다. 멤버 칩은 일반 탭과 **완전히 같은 마크업**(`renderTab` 공용 — 클릭·더블클릭·드래그·가운데 클릭 동일)이라 박스만 묶음을 말한다. ⚠️ 세그먼트(풀높이+구분선) 시절 시도한 장식형 묶음 표시 — 채움+연결선, 끝 라운드, 그룹 간 여백, 브래킷 캡 — 은 **전부 사용자 반려**(2026-08-10). 장식으로 돌아가지 말 것; 칩+컨테이너 구조가 확정안이다. ⌘1..9·⌃Tab 순회는 평탄화된 **표시 순서**(`tabView.tabs`)를 따른다.
