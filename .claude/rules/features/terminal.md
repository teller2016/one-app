...
- ⚠️ **`gap: 0` 이라 우측 변경사항 드로어는 스스로 `margin-left: 8px` 을 갖는다** — gap 을 없앤 목적은 xterm 좌측을 세션 패널에 붙이는 것(2026-08-06 사용자 요청)이지만 gap 은 `[main|changes]` 사이에도 걸린다. 그 8px 은 `.terminal__changes-grip`(absolute, `left: -8px`)이 놓일 자리이기도 하므로 지우면 손잡이가 xterm 위로 올라탄다. 좌측 간격은 `side-grip` 의 실폭 3px(width 11 + `margin: 0 -4px`)만 남는다.
