...
- 캐시 항목은 **조회한 Gitea 주소를 함께 기억**하고(`url`), 저장소 단위 캐시(기본 브랜치·base 후보·전체 이름)는 키에 주소를 넣는다 — 환경설정에서 서버를 바꾸면 옛 서버 결과가 TTL 동안 남지 않게(2026-09-07). PR 별 리뷰 조회(N+1)는 `mapLimit` 6 동시.
- ⚠️ **캐시를 버리는 곳**: 수동 새로고침(`force`)·PR 생성·머지. 목록을 바꾸는 API 를 새로 추가하면 `invalidatePrList()` 를 함께 부를 것. 무효화는 **in-flight 조회 합류도 끊고 세대 카운터를 올린다**(2026-08-31) — 캐시만 비우면 생성·머지 직후의 재조회가 "생성 전에 시작된" 조회에 합류해 stale 목록이 다시 60초 캐시로 앉는다.
...
**상세(우)**: `prs:merge-info` + `prs:branch-commits`(생성 미리보기와 같은 채널 재사용)로 방향·충돌·커밋·파일(+증감)을 표시하고, 방식(merge/squash/rebase) Segment + [머지] → `useConfirm` 확인 후 `/pulls/{n}/merge`. 충돌 여부는 merge-info 가 오기 전까지 목록의 `mergeable`(아래)로 우선 표시. 머지 성공 → 목록 갱신 + Jira 해결 제안(제목의 이슈 키 매칭). **별도 머지 모달은 없다**(패널로 흡수). 메타 줄(저장소 · #번호 · 작성자)에는 **제목의 Jira 이슈 키를 공용 `IssueChip`** 으로 붙인다(클릭 = 브라우저에서 이슈 열기 — 환경설정에 Jira 주소가 있을 때만. 없으면 제목에 이미 있는 키를 되풀이할 뿐이라 생략. 2026-09-08 사용자 선택: 목록 행에는 넣지 않음). 키 추출은 `shared/jira-url.ts` 의 `issueKeysIn`(해결 제안과 같은 함수).
