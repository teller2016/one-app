...
계정은 환경설정의 비즈박스 공용이고, **소속 문구도 환경설정('결재 소속')에서 온다** —
근무자 표의 소속 칸(야근)과 제목(휴가)에 그대로 들어간다. 입력 기본값은
`userData/approval.json`(평문)에 저장하고, 합치기 전 저장본(`overtime.json`)이 있으면 처음
읽을 때 승격한다.
...
예전 단독판(`standalone/overtime`)에서 이식했다. puppeteer 를 쓰지 않는 이유가 두 가지다.

> ℹ️ **단독 배포판 `standalone/lite`(One App Lite)는 이 폴더의 파일을 복사하지 않고 그대로
> import 한다**(2026-09-03 재구성 — 그 전엔 복사본이라 드리프트가 났다). 그래서 셀렉터·판정
> 로직은 여기 한 곳만 고치면 된다. 대신 **`window.oneApp.approval.*` 채널을 새로 쓰게 되면**
> `standalone/lite/src/preload/preload.ts`·`src/renderer/types/global.d.ts`(부분집합)도 늘려야
> 한다 — 빠지면 그쪽 `npm run typecheck` 가 잡는다. 소속 문구(`WORKER_DEPT`)는 상수를 직접
> 쓰지 말고 `store.ts` 의 `getWorkerDept()`(환경설정 '결재 소속' — 기본값 없음)를 거친다 —
> 단독판을 다른 챕터 동료가 쓰기 때문이다.
...
**제목 형식**은 `[종류] 성명_소속 (날짜·시간)` 이다 — **2026-09-03 사용자 지정**(그 전엔
`[연차] 정수범_8월 12일` 처럼 소속이 없고 날짜가 괄호 밖이었다). 이름과 소속을 밑줄로 잇고
괄호에 사용 날짜를 넣으며, 시차·반차는 정확한 시간대를, 대체휴가는 근거 휴일근무일을
괄호 안에 함께 적는다.

```
[연차]     정수범_FE챕터_플랫폼서비스사업부문 (9월 1일)
[연차]     정수범_FE챕터_플랫폼서비스사업부문 (9월 1일~9월 2일)   ← 여러 날
[시차]     정수범_FE챕터_플랫폼서비스사업부문 (9월 1일 09:00~10:00)  ← 시차_1시간·2시간 공통 태그
[반차]     정수범_FE챕터_플랫폼서비스사업부문 (9월 1일 09:00~14:00)  ← 오전·오후반차 공통 태그
[대체휴가] 정수범_FE챕터_플랫폼서비스사업부문 (9월 1일, 휴일근무일: 08/09)
```

소속은 **환경설정 '결재 소속' 한 곳에서만** 온다(`approval/store.ts` 의 `getWorkerDept()`) —
설정값을 그대로 쓰고 제목에서만 공백을 밑줄로 바꾼다(`FE챕터 플랫폼기술부문` → `FE챕터_플랫폼기술부문`).
소속 문구가 바뀌면 코드가 아니라 그 설정을 고친다.

- ⚠️ **소속을 쪼개 조립하지 말 것** — 처음엔 그룹웨어에서 읽은 챕터 + 설정값의 첫 단어를 이어
  붙였는데, 사용자가 설정에 `FE챕터 플랫폼기술부문` 을 넣자 챕터가 두 번 들어가
  **`정수범_FE챕터_FE챕터`** 가 됐다(2026-09-03 사용자 신고). 그래서 `parseApplicant` 는 이름만
  돌려주고 `VacationStatus` 에서 `chapter`·`division` 을 없앴다.
- ⚠️ **기본값(옛 `WORKER_DEPT` 상수)을 되살리지 말 것**(2026-09-03 사용자 요청으로 삭제) —
  코드에 사내 부문명을 박아 두면 단독판을 쓰는 다른 챕터 동료의 결재가 조용히 남의 소속으로
  올라간다. 비어 있으면 `ipc.ts` 가 **야근·휴가 작성을 막고**(`NO_DEPT`) 설정을 채우라고 안내하며,
  두 폼은 `lib/useApprovalDept.ts` 로 미리 배너를 띄운다(눌러 보고 실패하지 않게).
...
main 의 `formatVacationTitle`(그 함수의 별칭)이 같은 구현을 쓰므로 한 곳만 고치면 되고,
**단독 배포판(`standalone/lite`)도 같은 파일을 import** 하므로 함께 바뀐다.
`shared/approval-format.test.ts` 가 형식을 고정하고 있다 — 형식을 바꿀 때 함께 고칠 것.
같은 파일에 `formatHoursTotal`(야근 시간합계)·`KIND_DAY_FACTOR` 도 있다.

- 렌더러는 이름·소속을 모르는 동안 자리표시(`성명`·`소속`)를 보여준다. 사용자가 제목을 고쳤어도
  **자리표시가 남아 있으면 보내지 않고**(`hasTitlePlaceholder`) main 이 실제 값으로 다시 만든다.
  자리표시가 그대로 상신되면 곤란하기 때문이다.
