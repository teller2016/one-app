...
### ⚠️ 조회는 비싸다 — 90초 캐시 (`statusCache.ts`, 2026-08-26)
조회 한 번이 헤드리스 BrowserWindow 를 띄우는 왕복(수십 초)인데 위젯은 **마운트마다** 조회한다 — 특히 **MO(폰)는 탭을 오갈 때마다 새로 마운트**된다. `attendance:fetch` 는 90초 캐시(진행 중 조회도 공유 — 데스크톱과 폰이 동시에 물으면 왕복 하나로 합친다), 수동 새로고침만 `force=true`.

- ⚠️ **무효화는 `runAttendance()` 안**에 둔다(`action !== 'status'` 분기) — 위젯·**트레이**·**리마인더 알럿** 어느 경로로 찍어도 걸리게. 찍기 경로를 새로 만들 때 무효화를 따로 부를 필요가 없다.
- 캐시 모듈은 **값만 들고 있다** — 조회 함수를 부르지 않는다(부르면 `attend.ts` 와 import 가 순환한다). 실패한 결과는 캐시에 남지 않는다.
...
**알럿의 [오늘은 더 알리지 않기] 체크박스**(2026-08-27) — 체크하고 알럿을 닫으면(어느 버튼이든) 그 슬롯(출근/퇴근 각각)의 오늘 알림·반복이 멈춘다(`mutedToday` → `reminder-state.json` 의 `muted`). 연차·외근처럼 그날만 안 찍는 상황용이라 **다음 날 자동 복귀**하고, 영구히 끄려면 환경설정의 요일 슬롯을 끈다. 앱을 재시작해도 유지된다.

- **`repeat.enabled` 일 때만 체크박스를 노출**한다(`handleReminder(type, key, repeating)`) — 1회 알림은 어차피 다시 안 뜨므로 무의미하다.
- ⚠️ `mutedToday` 를 `doneToday` 와 합치지 말 것 — '찍은 게 확인됨' 과 '사용자가 껐음' 은 근거가 달라 로그·디버깅에서 구분이 필요하다(스킵 판정 자체는 tick 에서 둘 다 걸린다).
- ⚠️ **개발 인스턴스와 빌드 앱은 스케줄러를 각각 돌린다**(2026-08-27 실측) — `devInstance.ts` 가 가르는 건 포트·tmux 소켓·창 상태뿐이라, 둘 다 떠 있으면 **같은 시각에 알럿이 2개 뜨고 반복도 2벌** 돌며 `reminder-state.json` 을 서로 덮어쓴다. 그래서 **새 필드를 추가하면 옛 코드가 도는 쪽이 그 키를 지운다**(dev 에서 `muted` 를 써도 빌드 앱이 날렸다). 리마인더를 고친 뒤 실사용 검증을 할 때는 한쪽만 띄우거나 빌드까지 반영할 것.

**하루 단위 상태는 디스크에 남긴다**(2026-08-09) — `userData/reminder-state.json`(`date`·`done`·`muted`·`failSafe`·`lastAttempt`). 메모리에만 두면 **앱을 재시작하는 순간 "오늘은 이미 처리했다"는 기억이 사라져** 반복 알림이 처음부터 다시 발화하고 '하루 1회'인 실패 폴백 알림도 재발화한다(개발 중엔 재시작마다). 날짜가 다르면 복원하지 않고 버린다.
