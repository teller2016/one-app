...
**소리는 별개 축이다** — `main/lib/sound.ts` 의 `playSound()`, 어느 음인지는 `getNotifySound(kind)`. 알럿·토스트 없이 소리만 쓰는 알림도 있다(새 메일 — 뱃지가 이미 있어 화면을 가릴 이유가 없다).

**알림음 선택**(2026-09-16) — 환경설정 → 알림/터미널의 `Select`(자리마다 하나: 새 메일·터미널 입력대기). 저장은 `settings.json` 의 `sounds` 맵 한 곳이고(터미널 알림 **강도**는 종전대로 `terminal.json` `notifyLevel` — 강도와 음원은 다른 축이다), 채널은 `settings:sounds:list`·`:preview`·`:set`(데스크톱 전용 — 소리는 맥에서만 난다). ⚠️ **고르는 즉시 재생 + 즉시 저장**이다(테마·알림 강도와 같은 규칙 — 소리는 났는데 [저장] 전이라 안 남으면 어긋난다). ⚠️ 저장값은 읽을 때마다 존재 확인을 거쳐 **음원이 사라졌으면 기본값으로 떨어진다** — 개인 음원(`~/Library/Sounds`)을 지웠다고 알림이 무음이 되면 안 된다. 드롭다운은 목록 조회가 실패해도 **현재 선택값을 합쳐** 보여준다(빈 목록이면 설정이 날아간 것처럼 보인다).
...
- **프로브**(30초 주기 + 트리거 시): `/sbin/route -n get <서버IP>` 의 interface 가 `utun*` 이면 **루프 확정(즉시 사망)**, 아니면 `https://www.gstatic.com/generate_204` HEAD(시도 1회 5초) — 연속 2회 실패면 사망. ⚠️ **실효 시간은 두 배다**: `fetchWithTimeout` 이 HEAD 를 멱등으로 보고 타임아웃·네트워크 오류를 1회 재시도(0.4초 대기)하므로 프로브 1회 최악 ≈ 10.4초, "연속 2회 실패"는 HTTP 최대 4회 시도·재확인 간격 5초까지 약 26초다(오탐을 줄이는 방향이라 그대로 둔다). 서버 IP 는 `STATE … CONNECTED` 라인 5번째 필드. 프로브 도중 트리거(인터페이스 변화·주기)가 오면 삼키지 않고 `rerunRequested` 로 표시해 끝난 뒤 다음 틱에 한 번 더 돈다(connected 를 벗어나면 무효). **잠자기 복귀(`powerMonitor` `resume`, app ready 뒤 등록) 직후 15초(`RESUME_GRACE_MS`)는 판정을 건너뛰고 누적 실패를 0 으로** — 소켓 회복 전의 실패로 살아날 터널에 SIGHUP 을 보내지 않는다. 인터페이스 지문이 바뀌면 `networksetup` 라벨 캐시도 비운다(`resetInterfaceLabels` — 새 어댑터가 장치명으로 나오던 문제).
- **반응**: `VpnStatus.stale` → 위젯 점 fail + "응답 없음 · 재연결 필요" + [재연결]. 토스트는 `dedupeKey: 'vpn-health'` 한 장(응답 없음 → 재연결됨 이 같은 자리에서 바뀜, 자리를 비운 사이 일어난 일이라 sticky). 시크릿이 있으면 **`signal SIGHUP` 자동 재연결** — root 데몬이 살아 있어 관리자 인증이 다시 필요 없고 재인증은 기존 PASSWORD 핸들러가 TOTP 로 응답. 시크릿이 없거나 재연결이 실패하면 알럿(`notify`). ⚠️ 시크릿 없음 알럿은 `probing` 구간 안이라 **`void notify` 로 기다리지 않는다** — await 하면 모달을 닫을 때까지 감시가 멈춘다. 위젯은 시크릿이 없으면 **connected 에서도 OTP 칸을 항상** 둔다 — [재연결]이 항상 있는데 칸만 숨기면 눌러도 "OTP를 입력하거나…" 오류만 떴다.
- **경로 추종**: 프로브가 살아 있다고 해도 `route -n get default`(VPN 이 켜져 있어도 **물리** 기본 인터페이스를 준다 — 실측) 와 서버 경로의 인터페이스가 다르면(유선을 꽂았는데 터널은 와이파이 en0 에 남음) 같은 SIGHUP 으로 새 인터페이스로 옮긴다(`isDetoured`, 시크릿 있을 때만). ⚠️ **경로 추종에는 도달 게이트가 없다(둘 수 없다)** — 새 기본 네트워크가 서버에 못 닿는 격리 랜·캡티브 포털이면 SIGHUP 뒤 재시도 소진으로 데몬을 잃는 위험이 남는데, "새 인터페이스 **쪽 길로** 서버에 닿는가"를 root 없이 검사할 방법이 없다(2026-09-07 실측: `net.connect({localAddress})`·`nc -b <if>`·`ping -b <if>` 모두 목적지의 더 구체적인 경로 — 서버 /32 는 옛 인터페이스, 나머지는 `0/1`→utun — 가 다른 인터페이스에 있으면 그 인터페이스의 기본 경로로 폴백하지 않고 즉시 EHOSTUNREACH. 기본 인터페이스는 스코프된 default 경로가 없다). 게이트를 넣으면 경로 추종이 항상 막히므로 넣지 말 것. 알림 문구의 인터페이스는 `networksetup -listallhardwareports` 로 사람이 읽는 이름(USB 10/100/1000 LAN)으로 바꾼다. 재시도 가드 `followedFor` 는 **지문이 바뀔 때마다 해제**한다 — 지문 값으로 막으면 뽑았다 다시 꽂을 때 처음과 같은 지문이라 두 번째부터 옮기지 않는다(2026-09-07 실측 버그).
...
- ⚠️ **재연결 게이트(`reconnectDecision`) — 성공할 수 있을 때만 SIGHUP 을 보낸다.** 실패하면 `--connect-retry-max 2` 소진으로 데몬이 죽어 나중에 관리자 인증부터 다시 해야 하고, 그건 손대지 않았을 때(TCP 재전송·ping-restart 로 자가 회복)보다 나쁘다. 물리 기본 경로(`route get default`)가 없으면 대기 · `unreachable` 사망은 **서버 `IP:포트`(STATE 5·6번째 필드)에 TCP 가 3초 안에 닿는지** 확인해 안 닿으면 대기(라우터 재부팅·캡티브 포털·잠자기 복귀 직후) · 루프 사망은 서버 경로가 터널이라 도달 검사를 못 쓰니 물리 기본 경로만 본다. 대기 중엔 stale 표시만 하고 주기 프로브가 복구를 기다린다. 자동 SIGHUP 은 **60초 쿨다운**(접촉 불량 대비 — 순수 함수 `inReconnectCooldown(lastAt, now, gap)`, health.test.ts 가 경계를 검증), 재연결 대기 한도는 **90초**(60초는 접속 15초×2+인증에 근접). 재연결 중 사용자가 [연결 해제]를 누른 경우(`disconnected && wasDisconnectedByUser()`)만 실패 알럿을 생략한다 — ⚠️ `disconnected` 는 데몬이 사유 없이 죽어도(외부 kill·크래시) 같은 값이라 상태만으로 사용자 해제로 단정하면 안 된다. `openvpn.ts` 의 `userDisconnected` 는 `disconnectVpn` 진입에 true·연결/재연결 시작에 false 로 **다음 연결까지 남는** 플래그다(기존 `intentionalExit` 는 close 핸들러가 1회 소비하고 즉시 false 로 되돌려 microtask 뒤에 오는 reject 시점엔 못 쓴다 — 역할이 달라 합치지 않았다). **프로브·도달 확인(합쳐 십수 초) 사이 OpenVPN 이 스스로 RECONNECTING(`connecting`)으로 갔으면** await 뒤마다 `stillConnected()` 로 확인해 옛 터널의 판정으로 토스트·SIGHUP 을 내지 않고, `connecting` 상태의 실패 알럿도 생략한다 — 그 상태에서 `reconnectVpn` 을 부르면 "연결된 상태에서만" throw 로 헛된 모달이 떴다(2026-09-07 리뷰).
...
**제어 창 크기·제목** — `--window-width/height` 로 타이틀바까지 포함해 100x100 정사각형으로 줄이고(scrcpy 기본 256x256), `--window-title=` 로 제목을 비운다(빈 값이 그대로 먹는다 — 2026-09-01 실측). **타이틀바 자체는 남긴다** — `--window-borderless` 를 쓰면 창을 마우스로 옮기지도 닫지도 못한다.

- ⚠️ `--window-width/height` 는 **콘텐츠 영역** 크기다 — 100 을 주면 창은 100x132 가 된다(타이틀바 32). 창 전체를 정사각형으로 맞추려면 높이에서 타이틀바를 뺀다.
