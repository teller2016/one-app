...
`deploy:status:fetch` 에 **30초 캐시**(2026-08-26) — 섹션에 들어올 때마다 젠킨스를 대상 수만큼 다시 때리던 것을 막는다(실측 261ms → 0ms). **주기 조회는 `force=true` 로 우회**하고(최신성 담당), 트리거는 `invalidateDeployStatus(projectId)` 로 버린다. ⚠️ 진입 표시가 최대 30초 낡을 수 있는 대신 `watchBuild` 이벤트가 실시간 갱신을 담당한다 — 캐시를 늘리려면 그 균형을 먼저 볼 것.
