...
커밋 내역의 **커밋 해시는 Gitea 커밋 페이지로, 메시지 속 이슈 키(BBJ-1234)는 Jira 로 링크화**(환경설정의 Gitea/Jira 주소 사용, 미설정이면 평문 — 젠킨스가 기록한 저장소 주소는 내부망이라 호스트는 설정된 Gitea 주소로 치환). 티켓 칩은 공용 `IssueChip`(`renderer/components`), 키 패턴 `JIRA_KEY_RE`·이슈 URL `jiraIssueUrl` 은 `shared/jira-url.ts` 에 있다(2026-09-08 PR 상세와 공용화). 커밋 메시지 전용 추출(`extractIssueKeys`)만 `deploy/lib/format.ts` 에 남는다.
...
`deploy:status:fetch` 에 **30초 캐시**(2026-08-26) — 섹션에 들어올 때마다 젠킨스를 대상 수만큼 다시 때리던 것을 막는다(실측 261ms → 0ms). **주기 조회는 `force=true` 로 우회**하고(최신성 담당), 트리거와 **`watchBuild` 의 최종 상태(성공·실패·오류)** 는 `invalidateDeployStatus(projectId)` 로 버린다(2026-09-07 — 완료 뒤 30초 안에 섹션을 다시 열면 'building' 캐시가 이벤트 결과를 덮던 것). ⚠️ 진입 표시가 최대 30초 낡을 수 있는 대신 `watchBuild` 이벤트가 실시간 갱신을 담당한다 — 캐시를 늘리려면 그 균형을 먼저 볼 것.
