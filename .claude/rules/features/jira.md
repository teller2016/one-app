...
- **필터는 두 층**: 서버(JQL)는 `project IN (…)` + `labels IN (…)` + 기간(월/직접/전체 × 기준 필드
  생성·해결·갱신)을 자르고, **상태·담당자·유형은 받은 결과 안에서 facet 으로 거른다**(`MultiSelect`,
  선택지 = 결과에 실제로 있는 값 + 개수). 담당자 후보 API 는 인스턴스마다 달라 기대지 않는다.
  **레이블만 예외로 서버 조건**이다 — 아래 절 참고.
...
## 레이블은 **서버 조건**이다 (날짜 축으로는 잡히지 않는다)

이 팀의 배포 레이블은 `26/08/03_운영배포` 처럼 **날짜_분류** 형식이고, 8월에 만든 티켓에도
10월 배포 레이블이 붙는다 — 즉 **갱신일·생성일·해결일 어느 축과도 상관이 없다.** 그래서 레이블만은
facet(화면 필터)이 아니라 JQL 로 올려 보낸다(`labels IN (…)`). 그전에는 전체 조회 후 화면에서
거르는 수밖에 없었다(2026-09-03 사용자 지적).

- 선택지는 **고른 프로젝트가 실제로 쓰는 레이블**이다(`loadProjectLabels`) — `labels` 는 프로젝트
  스코프 필드가 아니고 `GET /rest/api/3/label` 은 인스턴스 전역이라, `project IN (…) AND labels
  IS NOT EMPTY` 를 **`fields=labels,created` 둘만** 받아 집계한다. 프로젝트를 안 고르면 전역 목록
  으로 폴백. 2026-09-04 실측: 전역 286개(460ms) → DFD 16개(490ms)로 좁혀진다.
- **정렬은 최신 우선**(사용자 요청) — 이름이 날짜면 그 날짜(`26/09/17…` → `2026-09-17`), 아니면
  그 레이블이 붙은 **티켓의 최신 생성일**. 집계에서 그 날짜가 공짜로 나오므로 이름이 날짜가 아닌
  레이블도 최신순에 낄 수 있다(`sortLabelsNewestFirst`). 화면은 이 순서를 그대로 쓴다.
- 화면은 조회 조건 줄의 `MultiSelect searchable` 하나다 — 팝오버 검색은 **부분 일치**라 `09` 로
  `26/09/17_운영배포` 가 걸린다. 200개까지 렌더하고 나머지는 "검색으로 좁히세요".
  고른 것은 **칩으로 조회 줄 아래**에 깔리고, 칩을 누르면 그 하나만 빠진다.
- 순서는 어디서나 **최신 우선으로 통일**한다 — 선택지·칩·JQL·복사 결과(`orderLabels`)와 결과 표의
  정렬 옵션 **'레이블별'**(`SORT_CMP.label`, 티켓의 대표 레이블 = 이름이 가장 큰 것). 고른 순서대로
  두면 보고서에 붙일 목록이 뒤죽박죽 읽힌다(2026-09-04 요청).
- 조회 조건에 레이블이 하나라도 있으면 **프로젝트 없이도 조회를 허용**한다 — 배포 레이블은
  프로젝트를 넘나드는 축이다(`buildReportJql`).
- 레이블을 처음 고르는 순간 **기간 제한을 끈다**(`periodMode='all'`) — 켜 둔 채로는 그 달에 갱신되지
  않은 티켓이 통째로 빠져서, 이 기능을 만든 이유가 사라진다. 사용자가 다시 켜면 경고 문구만 띄운다.

### ⚠️ 실측 함정 (2026-09-03~04)

| 함정 | 실측 | 대응 |
|------|------|------|
| 와일드카드가 안 통한다 | `labels ~ "26/08*"` → **0건** (`labels = "26/08/03_운영배포"` → 3건) | 값을 `IN` 으로 나열한다 |
| 자동완성은 15개에서 잘린다 | `suggestions?fieldName=labels` 는 `fieldValue` 가 무엇이든 최대 15개 | 후보 API 를 **쓰지 않는다** — `label` 전체 목록 + 화면 검색 |
| 자동완성 `value` 는 JQL 인용된 채로 온다 | `"\"26/08/03_운영배포\""` | (같은 이유로 안 쓴다. 다시 쓸 일이 생기면 인용을 벗겨야 한다) |

⚠️ `MultiSelect` 는 **같은 tick 에 두 번 토글하면 앞 선택이 유실**되던 문제가 있었다(onChange 가 값
계약이라 함수형 업데이트를 못 쓴다) — `pendingRef` 로 직전 결과를 누적한다. 레이블처럼 여러 개를
연달아 고르는 목록에서 드러난다(2026-09-04 실측: 3개 연속 클릭 → 1개만 남음).
